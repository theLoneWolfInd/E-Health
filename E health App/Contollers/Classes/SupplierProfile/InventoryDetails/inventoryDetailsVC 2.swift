...
class inventoryDetailsVC: UIViewController,UITableViewDelegate, UITableViewDataSource , UITextFieldDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            ...
            let cell:inventoryDetailsTableViewCell = tablView.dequeueReusableCell(withIdentifier: "cell8") as! inventoryDetailsTableViewCell
            ...
            cell.btnEditMedicine.tag = indexPath.row
            cell.btnDeleteMedicine.tag = indexPath.row
            
            cell.btnEditMedicine.addTarget(self, action: #selector(edit_medicine_click_method), for: .touchUpInside)
            cell.btnDeleteMedicine.addTarget(self, action: #selector(delete_medicine_click_method), for: .touchUpInside)
    ...
    @objc func edit_medicine_click_method(_ sender:UIButton) {
         ...
         */
        let push = UIStoryboard.init(name: "Main", bundle: Bundle.main).instantiateViewController(identifier: "AddMoreMedicinesId") as? AddMoreMedicines
        
        push!.edit_medicine = "edit_medicine"
        push!.getProfileInfo = "edit_medicine"
        
        push!.getDictOfThatMedicine = self.getDictInventoryDetails
    ...
    @objc func delete_medicine_click_method(_ sender:UIButton) {
        
        let btn:UIButton = sender
        print(btn.tag)
        
        let alert = UIAlertController(title: "Delete", message: "Are you sure you want to delete '"+(self.getDictInventoryDetails["name"] as! String)+"' ?", preferredStyle: .alert)
    ...
    @objc func delete_medicine() {
        ...
        if let person = UserDefaults.standard.value(forKey: "keyLoginFullData") as? [String:Any] {
            ...
            let x3 : Int = self.getDictInventoryDetails["diseaseId"] as! Int
            let myString3 = String(x3)
            
            
            
            
            let params = delete_medine_wb(action: "deletemedicine",
                                          diseaseId: String(myString3),
