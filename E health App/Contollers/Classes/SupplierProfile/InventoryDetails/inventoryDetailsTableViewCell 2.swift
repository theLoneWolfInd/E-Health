...
class inventoryDetailsTableViewCell: UITableViewCell,UITextFieldDelegate {
    ...
    @IBOutlet weak var btnEditMedicine:UIButton! {
        didSet{
            btnEditMedicine.layer.cornerRadius = 8
            btnEditMedicine.clipsToBounds = true
            btnEditMedicine.backgroundColor = .systemOrange
            btnEditMedicine.setTitle(" Edit", for: .normal)
            btnEditMedicine.setTitleColor(.black, for: .normal)
        }
    }
    
    @IBOutlet weak var btnDeleteMedicine:UIButton! {
        didSet{
            btnDeleteMedicine.layer.cornerRadius = 8
            btnDeleteMedicine.clipsToBounds = true
            btnDeleteMedicine.backgroundColor = .systemRed
            btnDeleteMedicine.setTitle(" Delete", for: .normal)
            btnDeleteMedicine.setTitleColor(.black, for: .normal)
            btnDeleteMedicine.tintColor = .white
        }
    }
